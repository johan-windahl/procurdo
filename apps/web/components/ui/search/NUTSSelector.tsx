"use client";

import { useCallback, useEffect, useMemo, useState } from "react";
import data from "@/data/nuts.json";

type NutsNode = {
  code: string;
  name: string;
  level: number;
  parent: string | null;
};

type Props = {
  value: string[];
  onChange: (codes: string[]) => void;
};

export function NUTSSelector({ value, onChange }: Props) {
  const [expandedNodes, setExpandedNodes] = useState<Set<string>>(new Set());

  const nodeMap = useMemo(() => {
    const map = new Map<string, NutsNode>();
    (data as NutsNode[]).forEach((node) => {
      map.set(node.code, node);
    });
    return map;
  }, []);

  const getChildrenForNode = useCallback(
    (nodeCode: string): NutsNode[] => {
      return (data as NutsNode[]).filter((node) => node.parent === nodeCode);
    },
    [],
  );

  const getAncestorCodes = useCallback(
    (code: string): string[] => {
      const ancestors: string[] = [];
      let currentNode = nodeMap.get(code);

      while (currentNode?.parent) {
        ancestors.push(currentNode.parent);
        currentNode = nodeMap.get(currentNode.parent);
      }

      return ancestors;
    },
    [nodeMap],
  );

  const topLevelNodes = useMemo(() => {
    return (data as NutsNode[]).filter((node) => node.level === 1);
  }, []);

  useEffect(() => {
    setExpandedNodes((prev) => {
      let changed = false;
      const next = new Set(prev);

      value.forEach((code) => {
        const node = nodeMap.get(code);
        if (!node) return;

        // Expand all ancestors
        getAncestorCodes(code).forEach((ancestor) => {
          if (!next.has(ancestor)) {
            next.add(ancestor);
            changed = true;
          }
        });

        // Expand the node itself if it has children
        const children = getChildrenForNode(code);
        if (children.length > 0 && !next.has(code)) {
          next.add(code);
          changed = true;
        }
      });

      return changed ? next : prev;
    });
  }, [value, getAncestorCodes, getChildrenForNode, nodeMap]);

  const toggleExpansion = (nodeCode: string) => {
    setExpandedNodes((prev) => {
      const next = new Set(prev);
      if (next.has(nodeCode)) next.delete(nodeCode);
      else next.add(nodeCode);
      return next;
    });
  };

  const toggleSelection = (node: NutsNode, checked: boolean) => {
    const codes = new Set(value);
    if (checked) codes.add(node.code);
    else codes.delete(node.code);
    onChange(Array.from(codes));
  };

  const renderNode = (node: NutsNode) => {
    const isExpanded = expandedNodes.has(node.code);
    const children = getChildrenForNode(node.code);
    const hasChildren = children.length > 0;
    const isSelected = value.includes(node.code);

    return (
      <li key={node.code} className="mb-1">
        <div className="flex items-center gap-2 rounded p-1 hover:bg-accent/40">
          {hasChildren ? (
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
                toggleExpansion(node.code);
              }}
              className="flex h-4 w-4 items-center justify-center text-muted-foreground hover:text-foreground"
              aria-expanded={isExpanded}
            >
              {isExpanded ? "▼" : "▶"}
            </button>
          ) : (
            <div className="w-4" />
          )}

          <input
            id={`nuts-${node.code}`}
            type="checkbox"
            checked={isSelected}
            onChange={(e) => toggleSelection(node, e.target.checked)}
            className="h-4 w-4"
          />

          <label htmlFor={`nuts-${node.code}`} className="cursor-pointer text-sm">
            <span className="mr-2 font-mono text-muted-foreground">{node.code}</span>
            {node.name}
          </label>
        </div>
        {hasChildren && isExpanded ? (
          <ul className="ml-4 mt-1">{children.map((child) => renderNode(child))}</ul>
        ) : null}
      </li>
    );
  };

  const clearAllSelections = () => {
    onChange([]);
    setExpandedNodes(new Set());
  };

  return (
    <div className="rounded-md border bg-card p-3">
      <div className="mb-2 flex items-center justify-between">
        <div className="text-sm font-medium">
          NUTS-koder ({value.length} valda)
        </div>
        {value.length > 0 ? (
          <button
            type="button"
            onClick={clearAllSelections}
            className="rounded border px-3 py-1 text-xs hover:bg-accent"
            title="Rensa alla val"
          >
            Rensa
          </button>
        ) : null}
      </div>
      <div className="max-h-96 overflow-y-auto">
        <ul className="space-y-1">
          {topLevelNodes.map((node) => renderNode(node))}
        </ul>
      </div>
    </div>
  );
}
